...
[Пример сайта](https://starburger.dwreaper.tk)
...
Определите переменную окружения `SECRET_KEY`. Создать файл `.env` в каталоге `star_burger/` и заполните её произвольным значением:
```sh
SECRET_KEY=
```
...
Для отслеживания ошибок на сайте используется система мониторига Rollbar. Зайдите в [панель управления](https://rollbar.com/). Откройте Projects → ВашПроект → Project Access Tokens и скопируйте токен `post_server_item`.
```sh
...
- `SECRET_KEY` — секретный ключ проекта. Он отвечает за шифрование на сайте. Например, им зашифрованы все пароли на вашем сайте. Заполняется произвольной строкой. Не рекомендуется использовать значение по умолчанию.
...
Для деплоя проекта и подтягивания изменений из репозитория запустите скрипт:
```sh
./deploy_star_burger.sh
```
Скрипт:
1. Обновит код репозитория
2. Установит библиотеки для Python и Node.js
3. Пересоберёт JS-код
4. Пересоберёт статику Django
5. Накатит миграции
6. Перезапустит сервисы Systemd
